/**
 * Utility to call the local backend proxy.
 * This is necessary because Vertex AI keys cannot be used directly in the frontend.
 */
...
- Does this character have WINGS? → If yes, describe them (e.g., "two tiny yellow wings"). If no, write "NO WINGS".
...
Return ONLY the JSON.`;
    ...
    const parsed = await callProxy('/api/analyze', { base64Image, prompt });
...
export async function generateImage(prompt: string, aspectRatio: "1:1" | "3:4" | "4:3" | "16:9" | "9:16" = "1:1", referenceImage?: string, retries = 5): Promise<string> {
  try {
    const result = await callProxy('/api/generate-image', { prompt, aspectRatio, referenceImage });
...
export async function generateText(prompt: string, retries = 3): Promise<string> {
  try {
    const result = await callProxy('/api/generate-text', { prompt });
    return result.text;
  } catch (error: any) {
...
export async function refinePrompt(currentPrompt: string, hint: string): Promise<string> {
  try {
    ...
    const systemInstruction = isReprompt
      ? `You are a creative packaging prompt engineer. The user wants to RE-IMAGINE or OPTIMIZE the current prompt without specific instructions.\n\nCURRENT PROMPT:\n---\n${currentPrompt}\n---\n\nYOUR TASK:\nRewrite the prompt to make it more creative, detailed, or visually interesting. Return ONLY the new prompt text.`
      : `Apply modification "${hint}" to prompt: ${currentPrompt}`;

    const result = await callProxy('/api/generate-text', { prompt: systemInstruction });
    return result.text;
  } catch (error) {
