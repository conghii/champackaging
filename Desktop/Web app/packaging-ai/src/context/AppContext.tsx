...
const TEMPLATE_VERSION = "v83"; // Bump this when prompts.json changes significantly
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const generateSide = async (side: keyof typeof generatedImages, template: string, aspectRatio: "1:1" | "3:4" | "4:3" | "16:9" | "9:16") => {
    ...
    try {
      ...
      // Give the user UI feedback instead of failing silently in console
      window.alert(`Lỗi khi tạo ảnh mặt ${String(side)}:\n\n${error.message || error}\n\n-> Nếu là lỗi Quota, vui lòng đợi 1-2 phút rồi thử lại từng mặt một.`);
    } finally {
