...
import { RefreshCw, ZoomIn, ZoomOut, Layers, Edit, MoreHorizontal, Loader2, X, Download, Save, RotateCcw, Box, Sparkles, FolderDown } from 'lucide-react';
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      const imagesToDownload = Object.entries(generatedImages)
        .filter(([_, url]) => url !== null) as [string, string][];

      if (imagesToDownload.length === 0) {
        alert("No images to download yet. Generate some first!");
        setIsDownloadingAll(false);
        return;
      }

      await Promise.all(
        imagesToDownload.map(async ([key, url]) => {
          try {
      ...
      saveAs(content, 'packaging-designs.zip');
    } catch (err) {
          ...
          <div className="max-w-[1600px] mx-auto flex flex-col gap-8">
            <div className="flex justify-end mb-4">
              <button
                ...
                disabled={isDownloadingAll || Object.values(generatedImages).every(url => url === null)}
                className={`flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-xl transition-all shadow-lg ${isDownloadingAll || Object.values(generatedImages).every(url => url === null)
                  ? 'bg-slate-800 text-slate-500 border border-slate-700 cursor-not-allowed opacity-50'
                  : 'bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-400 hover:to-purple-400 text-white shadow-indigo-500/25 hover:scale-105'
                  }`}
              >
                {isDownloadingAll ? (
                  <><Loader2 className="size-4 animate-spin" /> Zipping...</>
                ) : (
                  <><FolderDown className="size-4" /> Download All</>
                )}
              </button>
            </div>
